...
### 🧠 Overview
To ensure the reliability of the **Sweet Shop Management System**, Test-Driven Development (TDD) was implemented for core modules such as **Billing**, **Sales**, and **Inventory Management**.  
This approach followed the classic *Red → Green → Refactor* cycle:
1. **Write a failing test** (Red)
2. **Implement code to make it pass** (Green)
3. **Refactor** to improve code quality without breaking functionality

---

### 🧾 Test Cases Implemented (TDD)

| 🧩 **Test Class** | 🧪 **Test Case** | 🧠 **Purpose** | ✅ **Result** |
|-------------------|------------------|----------------|----------------|
| **BillingServiceTDDTest** | `testCalculateTotal_ValidInput()` | Verifies the correct total amount calculation for given sweet items | ✅ Passed |
| **BillingServiceTDDTest** | `testCreateSale()` | Ensures sale creation assigns correct user and total | ✅ Passed |
| **BillingServiceTDDTest** | `testProcessSale_UpdatesStock()` | Validates that the stock quantity is updated after each sale | ✅ Passed |

---

### 🧰 Mock Classes Used
To isolate the logic and avoid database dependency during testing, **mock DAO classes** were created under  
`src/test/java/com/sweetshop/mockdao/`:
- **MockSalesDAO.java** → Simulates sale persistence.  
- **MockSweetItemDAO.java** → Mimics inventory updates and quantity management.

Similarly, lightweight **mock models** were placed under  
`src/test/java/com/sweetshop/mockmodel/` for independent test execution.
...
Run the tests using **JUnit 5 Console Launcher**:
